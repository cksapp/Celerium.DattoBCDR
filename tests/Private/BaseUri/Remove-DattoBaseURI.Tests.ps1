<#
    ...
    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\BaseUri\Remove-DattoBCDRBaseURI.Tests.ps1

        Runs a pester test and outputs simple results

    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\BaseUri\Remove-DattoBCDRBaseURI.Tests.ps1 -Output Detailed
...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('BaseUri') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "The BaseUri variable should not longer exist" {
            ...
            $DattoBCDRModuleBaseUri | Should -BeNullOrEmpty
        }

        It "If the BaseUri is already gone a warning should be thrown" {
            Add-DattoBCDRBaseURI
            Remove-DattoBCDRBaseURI
            Remove-DattoBCDRBaseURI -WarningAction SilentlyContinue -WarningVariable BaseUriWarning
