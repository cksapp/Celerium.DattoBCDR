<#
    ...
    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\ApiKeys\Remove-DattoBCDRAPIKey.Tests.ps1

        Runs a pester test and outputs simple results

    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\ApiKeys\Remove-DattoBCDRAPIKey.Tests.ps1 -Output Detailed
...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('ApiKeys') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "Running [ $commandName ] should remove all apiKey variables" {
            ...
            $DattoBCDRModuleApiKey | Should -BeNullOrEmpty
            $DattoBCDRModuleApiSecretKey | Should -BeNullOrEmpty
        }

        It "If the [ DattoBCDRModuleApiKey ] is already empty a warning should be thrown" {
            Add-DattoBCDRAPIKey -ApiKeyPublic '12345' -ApiKeySecret "Celerium.DattoBCDRAPIKey"
            Remove-Variable -Name "DattoBCDRModuleApiKey" -Scope Global -Force

            Remove-DattoBCDRAPIKey -WarningAction SilentlyContinue -WarningVariable apiKeyWarning
            $apiKeyWarning | Should -Be "The Datto API [ public ] key is not set. Nothing to remove"
        }

        It "If the [ DattoBCDRModuleApiSecretKey ] is already empty a warning should be thrown" {
            Add-DattoBCDRAPIKey -ApiKeyPublic '12345' -ApiKeySecret "Celerium.DattoBCDRAPIKey"
            Remove-Variable -Name "DattoBCDRModuleApiSecretKey" -Scope Global -Force
