...
Describe "Testing the [ $buildTarget ] version of [ $commandName ] functions with [ $pester_TestName ]" -Tag @('ApiKeys') {
    ...
    Context "[ $commandName ] testing functions" {
        ...
        It "[ $commandName ] should have an alias of [ Set-DattoBCDRAPIKey ]" {
            Get-Alias -Name Set-DattoBCDRAPIKey | Should -BeTrue
        }

        It "[ -ApiKeySecret ] should accept a value from the pipeline" {
